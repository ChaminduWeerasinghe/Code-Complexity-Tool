 ...
 <%
 	if(request != null)
 	{
		String loc = DataModel.map.get(request.getParameter("fileName"));
 		calcSizeVariableMethod calc = new calcSizeVariableMethod();
 		DATAofSizeVariableMethod data =  calc.mainMethod(loc);
...
<h2><b>Complexity due to SIZE</b></h2>
	<table class="table table-hover table-dark">
	 <thead>
	  ...
	  <tbody>
		<%
		DATAofSizeVariableMethod data = (DATAofSizeVariableMethod)session.getAttribute("data");
		for(int  i = 1; i < data.string.size() + 1; i++)
		{
	 ...
	 <thead>
	  ...
	  <tbody>	
		...
		for(int  i = 1; i < data.string.size() + 1; i++)
		{
			...
			{
				compositeGlobal = data.compositeGlobalVariable.get(i);
				variableScope = 2;
				displayScope = "2";
			}
			else if(data.compositeLocalVariable.get(i) != null)
			{
				compositeLocal = data.compositeLocalVariable.get(i);
				variableScope = 1;
				displayScope = "1";
			}
